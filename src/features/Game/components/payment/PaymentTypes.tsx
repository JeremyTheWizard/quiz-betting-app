...
const PaymentTypes = () => {
  ...
  const getUserBalance = useCallback(async () => {
    const userObject = await fcl
      .send([fcl.getAccount(user.magic.addr ?? user.fcl.addr)])
      .then(fcl.decode);
  ...
  return (
    <TabGroup>
      <Tab.List>
        <Carousel
          className='left-[50%] w-screen -translate-x-2/4'
          itemWrapperClassName='w-max'
          scrollContainerClassName='snap-none pl-[5vw]'
          slide={false}
          indicators={false}
        >
            ...
            <div className='grid grid-cols-2 items-center justify-between gap-2'>
              ...
              >
                <span className='mx-auto w-full'>
                  {addressFormatter(user.magic?.addr ?? user.fcl.addr)}
                </span>
              </Button>
            </div>
            <div className='grid grid-cols-2  items-center justify-between gap-2'>
              ...
              <Button
                ...
                rightIconClassName='text-primary-500 text-xl'
                size='base'
                className='w-full px-5 py-3 text-white disabled:bg-transparent'
              >
                <span className='mx-auto w-full'>
